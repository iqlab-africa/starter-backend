...
const mm = "🥝 🥝 🥝 IQLab Backend App 🍎";
console.log(`${mm} ... starting up ...`);

const firebaseManager: FirebaseManager = new FirebaseManager();
async function bootstrap() {
  Logger.log(`${mm} ... IQLab Backend (NestJS) bootstrapping .....`);

  const nestApp = await NestFactory.create(AppModule);
  const port = MyUtils.getPort();
  const environment = process.env.NODE_ENV;
  Logger.log(
    `${mm} ... IQLab Backend running on port : ${port} environment: ${environment}`
  );
...
bootstrap().then((r) =>
  Logger.log(
    `${mm} IQLab Backend Bootstrapping is complete. 💖 💖 💖 ... Let's do this!!!`
  )
);
